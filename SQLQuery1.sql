...
--	RETURN THE NUMBER OF ROWS WITHIN A WINDOW
	...
	*/
	...
	-- AND HIGHEST & LOWEST SALES FOR EACH PRODUCT
	...
	SELECT
		...
		PRODUCTID,
		SALES,
		MIN(SALES) OVER() MINPRODUCT,
		MAX(SALES) OVER() MAXPRODUCT,
		MAX(SALES) OVER(PARTITION BY PRODUCTID) MINSALES,
		MIN(SALES) OVER(PARTITION BY PRODUCTID) MAXSALES		
		FROM SalesDB.Sales.Orders;

	-- SHOW THE EMPLOYEES WHO HAS HIGHEST SALARIES
	SELECT * FROM(
	SELECT
		*,
	...
	SELECT
		...
		SALES - MIN(SALES) OVER() DEVIATIONSALESMIN,
		MAX(SALES) OVER() - SALES DEVIATIONSALESMAX
		FROM Sales.Orders;


	/*
	RUNING TOTOL AND ROLLING TOTAL:
		THEY AGGREGRATE SEQUENCE OF MEMBERS AND THE AGGREGRATION IS UPDATED EACH TIME A NEW MEMBER IS ADDED
	RUNNING TOTAL:
		AGGEGRATE ALL VALUES FROM BEGGINING UPTO THE CURRENT POINT WITHOUT DROPPING OD OLDER DATA
	ROLLING TOTAL:
		AGGEGRATE ALL VALUES WITHIN A FIXED TIME WINDOW(EG. 30DAYS) AS NEW DATA IS ADDED, THE OLDEST DATA PINT WILL BE DROPPED.
	*/
	--CALCULATE MOVING AVEARGE OF SALES FOR EACH PRODUCT OVER TIME AND ONLY NEXT ORDER
	SELECT
		ORDERID,
		PRODUCTID,
		ORDERDATE,
		SALES,
		AVG(SALES) OVER(PARTITION BY PRODUCTID) AVERAGE,
		AVG(SALES) OVER(PARTITION BY PRODUCTID ORDER BY ORDERDATE) AS RUNNINGTOTAL,
		AVG(SALES) OVER(PARTITION BY PRODUCTID ORDER BY ORDERDATE
						ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS ROLLINGTOTAL
		FROM Sales.Orders;
